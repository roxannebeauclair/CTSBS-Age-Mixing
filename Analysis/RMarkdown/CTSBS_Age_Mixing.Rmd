...
tidysummary <- impsex %>% # A model will be applied to all imputations and sex
  mutate(model = map(data, ~ bwmodel(.x)), # bwmodel is function for nb bw model, model as stored as var in nested df
         ...
         splinepred = map2(data, model, splinepreds)) # Create predictions across dif vals of age (our spline var) and store as
...
part <- partdfimp %>% # Take original participant-level imputed df
  filter(relcount > 1) %>% # Take out participants who had more than one rel
...
menEBWR <- men[2, 1] # Extract HIV EBWR 
menEBWRlwr <- men[2, 6] # Extract HIV lower CI limit
menEBWRupr <- men[2, 7] # Extract HIV upper CI limit
womenEBWR <- women[2, 1]
womenEBWRlwr <- women[2, 6]
womenEBWRupr <- women[2, 7]
```
