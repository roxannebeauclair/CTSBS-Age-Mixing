...
tidysummed <- impsexad %>%
  mutate(modelad = map2(data, sex, admodel),
         modelgamad = map(data, adgammodel),
         modelsumad = map(modelad, ~tidy(.x, effects = "fixed")),
         modelgamsumad = map(modelgamad, tidygamm),
         splinepredad = map(modelad, lmesplinepreds),
         smoothpredad = map(modelgamad, gammsmoothpreds))
...
admodgamcoef <- tidysummed %>%
  unnest(modelgamsumad, .drop = TRUE)
...
# Unnest the tidy summary, to see the tidy model predictions for values of age
# for each gamm model
admodgampred <- tidysummed %>%
  unnest(smoothpredad, .drop = TRUE)
...
```{r pool_betas_mods, include = FALSE}
# Now we want to produce a pooled beta for HIV according to 
# Rubin's rules. To do this we use mice functions
...
###### **Figure 10. Effect of participant HIV on age-differences in relationships, stratified by gender. Beta-coefficients are from linear mixed effects models with a random intercept for participant. This is for only relationships starting in the previous 12 months (n = 399). Models adjust for race and age.**
...
###### **Figure 11. Predicted age-differences for different ages of the participant, stratified by gender. The predictions are from linear mixed effects models with a random intercept for participant. This is for only relationships starting in the previous 12 months (n = 399: n = 230 and n = 169 for men and women, respectively). Models adjust for race and hiv status. The model for men contained a natural spline for age with 4 degrees of freedom as indicated by a preliminary GAMM, while the model for women had a spline with 2 df.**
