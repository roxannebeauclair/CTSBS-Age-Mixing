...
# GAM models
# Unfortunately, gamm does not allow mixed effects with negative binomial regression
...
bwmodpoolm <- part %>%
  with(
    glm.nb(bridgewidth ~ hiv + splines::ns(age, 2) + race,
              subset = (sex == "Male"))
    ) %>%
...
```{r age_smooth, include = FALSE}
# Determine the degrees of freedom for the age spline in models
# Randomly select one dataset
set.seed(6022)
n_ad <- sample(1:imp, 1)
...
# Ultimately we want to know that the participant had an HIV status 
# before they started the relationship
dfad <- dfimp %>%
  filter(.imp == n_ad, 
         start == "Last year")
...
admodm <- gamm4(agedif ~ s(age) + hiv + race, 
           data = dfadm, 
           random = ~( 1 | id))

admodw <- gamm4(agedif ~ s(age) + hiv + race, 
           data = dfadw, 
           random = ~(1 | id))

summary(admodm$gam) # EDF is rounded to 4
summary(admodw$gam) # EDF is rounded to 2
```


```{r ad_med_setup, include = FALSE, cache = TRUE}

# Create grouped dataframe, without complete cases dataset


####========Models not converging with gams...so for now leaving all relationships in
...
# Does HIV predict condom use in relationships (adjusting for mediator agegap)?
# Does HIV predict sex freq in relationships (adjusting for mediator agegap)?
# Does HIV predict MCP in relationships (adjusting for mediator agegap)?
# Does HIV predict Rel duration in relationships (adjusting for mediator agegap)?

# In the past 12 months
# Model must use lmer (instead of lme) because of mediation analyis

tidysummed <- impsexad %>%
  mutate(#modelad = map2(data, sex, admodel),
         modelad = map(data, adgammodel),
         modelcf = map(data, ~sbmodel(.x, yvar = cf)),
         modelsf = map(data, sfmodel),
         #modelmcp = map(data, ~sbmodel(.x, yvar = relconcur)),
         #modelsumad = map(modelad, ~tidy(.x, effects = "fixed")),
         modelsumad = map(modelad, tidygam),
         modelsumcf = map(modelcf, tidygam),
         modelsumsf = map(modelsf, tidygam),
         #modelsummcp = map(modelmcp, tidygam),
         #splinepredadage = map(modelad, lmesplinepreds),
         smoothpredadage = map2(modelad, data, ~gammsmoothpreds(.x, .y, xvar = "age")),
         smoothpredcfage = map2(modelcf, data, ~gammsmoothpreds(.x, .y, xvar = "age")),
         smoothpredcfagedif = map2(modelcf, data, ~gammsmoothpreds(.x, .y, xvar = "agedif")),
         smoothpredsfage = map2(modelsf, data, ~gammsmoothpreds(.x, .y, xvar = "age")),
         smoothpredsfagedif = map2(modelsf, data, ~gammsmoothpreds(.x, .y, xvar = "agedif")))
         #smoothpredmcpage = map2(modelmcp, data, ~gammsmoothpreds(.x, .y, xvar = "age")),
         #smoothpredmcpagedif = map2(modelmcp, data, ~gammsmoothpreds(.x, .y, xvar = "agedif")))

tidysummed

# Unnest the tidy summary, to see the tidy model estimates for each model
# admodcoef <- tidysummed %>%
  # unnest(modelsumad, .drop = TRUE) %>%
  # ungroup() %>%
  # mutate(lwr = estimate - 2 * std.error,
         # upr = estimate + 2 * std.error)
...
# Unnest the tidy gamm summary, to see the tidy model estiamtes for each model
admodcoef <- tidysummed %>%
  unnest(modelsumad, .drop = TRUE)
...
# Unnest the tidy summary, to see the tidy model predictions for values of age
# for each model
# admodpred <- tidysummed %>%
  # unnest(splinepredad, .drop = TRUE)

# admodpred

# Unnest the tidy summary, to see the tidy model predictions for values of age
# for each gamm model
admodpredage <- tidysummed %>%
  unnest(smoothpredadage, .drop = TRUE)
...
cfmodpredage <- tidysummed %>%
  unnest(smoothpredcfage, .drop = TRUE)

cfmodpredage

# Unnest the tidy summary, to see the tidy model predictions for values of agegaps
# for each gamm model
cfmodpredagedif <- tidysummed %>%
  unnest(smoothpredcfagedif, .drop = TRUE)
...
sfmodpredage <- tidysummed %>%
  unnest(smoothpredsfage, .drop = TRUE)

sfmodpredage

# Unnest the tidy summary, to see the tidy model predictions for values of agegaps
# for each gamm model
sfmodpredagedif <- tidysummed %>%
  unnest(smoothpredsfagedif, .drop = TRUE)

sfmodpredagedif


# Unnest tidy summary to see model estimates with mcp as outcome
# mcpmodcoef <- tidysummed %>%
  # unnest(modelsummcp, .drop = TRUE)


# Unnest tidy summary to see model predictions for age with
# mcp as outcome

# mcpmodpredage <- tidysummed %>%
  # unnest(smoothpredmcpage, .drop = TRUE)



# Unnest tidy summary to see model predictions for agegaps
# with mcp as outcome

# mcpmodpredagedif <- tidysummed %>%
  # unnest(smoothpredmcpagedif, .drop = TRUE)

```


```{r pool_betas_ad_mods, include = FALSE}
# Now we want to produce a pooled beta for HIV according to 
# Rubin's rules. To do this we use mice functions

rels <- dfimp %>% # Take original rel-level imputed df
  # filter(start == "Last year") %>% # Only relationships starting last year
  as.mids() # Create mids object

admodpoolm <- rels %>%
  with(
    lmer(agedif ~ hiv + ns(age, df = 4) + race + (1 | id),
        subset = sex == "Male")
    ) %>%
  pool() %>% # Pool coefficients according to Rubin's Rules
  summary() %>%
...
admodpoolw <- rels %>%
  with(
    lmer(agedif ~ hiv + ns(age, df = 2) + race + (1 | id),
        subset = sex == "Female")
    ) %>%
  pool() %>% # Pool coefficients according to Rubin's Rules
  summary() %>%
...
menB <- admodpoolm[2, 1] # Extract HIV B 
menBlwr <- admodpoolm[2, 6] # Extract HIV lower CI limit
menBupr <- admodpoolm[2, 7] # Extract HIV upper CI limit
womenB <- admodpoolw[2, 1]
womenBlwr <- admodpoolw[2, 6]
womenBupr <- admodpoolw[2, 7]
```
...
label <- c(
  paste0("Pooled B-Coefficient: ", menB, "(", menBlwr, " - ", menBupr, ")"),
  paste0("Pooled B-Coefficient: ", womenB, "(", womenBlwr, " - ", womenBupr, ")")
)


adhivfig <- admodcoef %>%
  filter(term == "hivPositive") %>%
  ggplot(aes(x = .imp, estimate)) +
   geom_hline(yintercept = 0, 
             color = "blue") +
  geom_point() +
  geom_pointrange(aes(ymax = upr,
                      ymin = lwr)) +
  facet_grid(sex ~ .) +
  annotate("text", x = 38, y = -9, label = label) +
  xlab("Imputation") +
  ylab("B-Coefficient") +
  theme
...
<br>
```{r ad_age_fig, echo = FALSE}

adagefig <- admodpredage %>%
  ggplot(aes(x = age, y = pred, group = .imp)) +
  geom_line() +
  facet_grid(. ~ sex) +
  xlab("Age") +
...
###### **Figure 19. Effect of HIV on average number of times sex occurred per week in the relationship, stratified by gender. Incidence Rate Ratios (IRRs) are from generalized additive mixed models (poisson outcome) with a random intercept for the participant. Models are adjusted for age, agegaps (hypothetical mediator) and race.**
...
###### **Figure 20. Predicted average number of times sex occurred per week in a relationship for different ages of participant, stratified by gender. The predictions are from generalised additive mixed models (poisson outcome) with a random intercept for the participant. Models are adjusted for hiv status, age gaps(hypothetical mediator) and race.**
...
###### **Figure 21. Predicted average number of times sex occurred per week in a relationship for different agegaps, stratified by gender. The predictions are from generalised additive mixed models (poisson outcome) with a random intercept for the participant. Models are adjusted for hiv status, age, and race.**

<br>

```{r sf_agedif_fig, echo = FALSE}

sfagediffig <- sfmodpredagedif %>%
  ggplot(aes(x = agedif, y = pred, group = .imp)) +
  geom_line() +
  facet_grid(. ~ sex) +
  xlab("Age difference") +
