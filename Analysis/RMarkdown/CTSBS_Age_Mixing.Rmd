...
tidysumad <- impsexad %>%
  mutate(model = map2(data, sex, admodel),
         ...
         splinepred = map(model, lmesplinepreds))
...
###### **Figure 11. Predicted age-differences for different ages of the participant, stratified by gender. The predictions are from linear mixed effects models with a random intercept for participant. This is for only relationships starting in the previous 12 months (n = 399). Models adjust for race and hiv status. The model for men contained a natural spline for age with 4 degrees of freedom as indicated by a preliminary GAMM.**
