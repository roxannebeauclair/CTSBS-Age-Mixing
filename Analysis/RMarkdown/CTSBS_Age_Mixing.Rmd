...
comb <- bind_rows(dfampm, dfampw) %>%
  mutate(population = "General population")
...
comb2 <- bind_rows(dfamphivm, dfamphivw) %>%
  mutate(population = "HIV")

# Now add the HIV dataset to the general population dataset
...
###### **Figure 1. Age mixing pattern for randomly selected imputed dataset. Linear mixed effects model without hetero skedastic variance explicitly modeled. The top row represents the male and female age mixing patterns in the whole study population, while the bottom row is for only those with HIV**
...
# gender, imputation, and whether they are in the general population
# versus HIV population. So with 2 genders, 50 imps, and 2 pop status,
...
impsexpopamp <- dfimp %>% 
  ...
  mutate(population = "General population") %>%
  bind_rows(hivamp) %>%
  group_by(.imp, sex, population) %>%
  nest()

# Produce nice summary of the models
# A model will be applied to all imputations, sex, and population subgroups
tidysumamp <- impsexpopamp %>%
...
```{r amp_model_slope_plot, echo = FALSE}
...
```{r amp_model_intercept_plot, echo = FALSE}
...
```{r amp_model_wvar_plot, echo = FALSE}
...
```{r amp_model_bvar_plot, echo = FALSE}
