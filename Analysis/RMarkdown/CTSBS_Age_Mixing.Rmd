...
n_race <- nrow(df2) - nrow(df3)

# How many exclusions due to no parter reported
df4 <- df3 %>%
  filter(partner != 0) 

n_nopart <- nrow(df3) - nrow(df4)
...
# WSSD(age) = sqrt( (WSSD(baseline) ^ 2) * (1+age_above_baseline)^PowerCoeff)
# age_above_baseline = 40 - 15, if doing for 40 year olds

# Unnest tidy summary for data on within individual variance
modwvar <- ampdf %>%
  unnest(wvar, .drop = TRUE) %>%
  mutate(wsd40 = sqrt(wsd ^ 2 * (26 ^ p))) 

modwvar

wvardif <- modwvar %>%
  select(-lwrwsd, -uprwsd, -lwrpsd, -uprpsd) %>%
  unite(subpop, sex, hiv) %>%
  gather(items, value, wsd, p, wsd40) %>%
  unite(subpopitem, subpop, items) %>%
  spread(subpopitem, value) 
...
difwvarsum <- wvardif %>%
  mutate(gendifneg = Female_Negative_wsd - Male_Negative_wsd, # Ha: Female wvar larger than males
         hivdifmale = Male_Positive_wsd - Male_Negative_wsd, # Ha: Pos wvar larger than neg
         gendifneg40 = Female_Negative_wsd40 - Male_Negative_wsd40,
         hivdifmale40 = Male_Positive_wsd40 - Male_Negative_wsd40) 
...
wsdci <- modwvar %>%
  ...
  summarise(mean = mean(wsd),
            ...
            mean40 = mean(wsd40, na.rm = T),
            lwr40 = quantile(wsd40, alpha / 2, na.rm = T),
            upr40 = quantile(wsd40, 1 - alpha /2, na.rm = T))
...
wvardifci <- difwvarsum %>%
  summarise(meangendifneg = mean(gendifneg),
            meanhivdifmale = mean(hivdifmale),
            meangendifneg40 = mean(gendifneg40),
            meanhivdifmale40 = mean(hivdifmale40),
            lwrgendifneg = quantile(gendifneg, alpha / 2),
            lwrhivdifmale = quantile(hivdifmale, alpha / 2),
            lwrgendifneg40 = quantile(gendifneg40, alpha / 2),
            lwrhivdifmale40 = quantile(hivdifmale40, alpha / 2),
            uprgendifneg = quantile(gendifneg, 1 - alpha / 2),
            uprhivdifmale = quantile(hivdifmale, 1 - alpha / 2),
            uprgendifneg40 = quantile(gendifneg40, 1 - alpha / 2),
            uprhivdifmale40 = quantile(hivdifmale40, 1 - alpha / 2)) %>%
  mutate_all(funs(round(., 2)))
...
powerci <- modwvar %>%
  ...
  filter(sex == "Male" |
           hiv == "Negative") %>%
  group_by(sex, hiv) %>%
  summarise(mean = mean(p, na.rm = T),
            lwr = quantile(p, alpha / 2, na.rm = T),
            upr = quantile(p, 1- alpha / 2, na.rm = T))
...
wvarplot <- modwvar %>%
  ggplot(aes(x = wsd)) +
  geom_density(alpha = 0.2, fill = "blue") +
  geom_density(aes(x = wsd40), alpha = 0.2, fill = "yellow") +
  geom_vline(aes(xintercept = mean), 
             data = wsdci,
             linetype = "dashed") +
  geom_vline(aes(xintercept = mean40),
             data = wsdci,
             linetype = "dotted") +
  geom_vline(aes(xintercept = lwr), 
             data = wsdci,
             linetype = "dashed") +
  geom_vline(aes(xintercept = lwr40),
             data = wsdci,
             linetype = "dotted") +
  geom_vline(aes(xintercept = upr), 
             data = wsdci,
             linetype = "dashed") +
  geom_vline(aes(xintercept = upr40),
             data = wsdci,
             linetype = "dotted") +
...
* Male and female wsd in the HIV negative population of 15 year olds was `r wvardifci$meangendifneg` (95% CI: `r wvardifci$lwrgendifneg` - `r wvardifci$uprgendifneg`)
* Male and female wsd in the HIV negative population of 40 year olds was `r wvardifci$meangendifneg40` (95% CI: `r wvardifci$lwrgendifneg40` - `r wvardifci$uprgendifneg40`)
* HIV positive and negative wsd in the male population of 15 year olds was `r wvardifci$meanhivdifmale` (95% CI: `r wvardifci$lwrhivdifmale` - `r wvardifci$uprhivdifmale`)
* HIV positive and negative wsd in the male population of 40 year olds was `r wvardifci$meanhivdifmale40` (95% CI: `r wvardifci$lwrhivdifmale40` - `r wvardifci$uprhivdifmale40`)
