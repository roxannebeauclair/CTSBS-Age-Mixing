...
```{r race_check, include = FALSE, warning = FALSE}
# Check in complete cases if race should be in the model
racecheckhnm <- dfimp %>%
  filter(.imp == 0,
         sex == "Male",
         hiv == "Negative") 
...
hnm <- lmer(agep ~ age0 + race + (1 | id),
       data = racecheckhnm,
       REML = TRUE)
tidy(hnm)
confint(hnm)

hpm <- lmer(agep ~ age0 + race + (1 | id),
       data = racecheckhpm,
       REML = TRUE)
...
###### **Figure 4. Distribution of age-mixing between-subject standard deviations (BSSD) for 50 imputed datasets, each bootstraped 1000 times.**
...
###### **Figure 5. Distribution of age-mixing within-subject standard deviations (WSSD) for 50 imputed datasets, each bootstraped 1000 times.**
...
mx %>%
  htmlTable(caption = "<b>Table 1. Differences between age-mixing pattern model features.</b>",
            tfoot = txtMergeLines("HNM, HIV Negative Men", 
                                 ...
                                 "These differences are calculated from 100,000 datasets"),
